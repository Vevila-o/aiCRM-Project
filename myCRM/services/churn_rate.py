...
def _risk_from_rfm(row: Dict[str, Any]) -> float:
    """無模型時的簡易風險分數（0~1），以分段正規化估計。"""
    r = max(0, float(row.get("recency_days", 0)))
    f = max(0, float(row.get("frequency", 0)))
    m = max(0, float(row.get("monetary", 0.0)))

    # 粗略正規化，避免除以零：
    r_norm = min(1.0, r / 180.0)  # 180 天以上視為高風險
    f_norm = 1.0 - min(1.0, f / 6.0)  # 6 次/月以內
    m_norm = 1.0 - min(1.0, m / 10000.0)  # 金額 1 萬為單位
...
def train_churn_model(
    ...
    # 取正類機率（AUC/F1/Precision/Recall等要用）
    try:
        proba_val = model.predict_proba(X_val)
        y_val_prob = [float(p[1]) if isinstance(p, (list, tuple)) else float(p) for p in proba_val]
    except Exception:
        y_val_prob = None
...
def predict_churn(
    ...
    if use_model:
        ...
        X = [[d["recency_days"], d["frequency"], d["monetary"]] for d in rfm]
        proba = model.predict_proba(X)
